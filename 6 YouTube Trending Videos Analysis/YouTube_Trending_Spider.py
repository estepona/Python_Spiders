...
if not new_file_name.is_file():
    df = pd.read_csv('Trending Videos.csv')
    df = add_new(df)
    df = check_old(df)
    df.to_csv('Trending Videos_new.csv')
else:
    # update two files
    df = pd.read_csv('Trending Videos_new.csv')
    df = add_new(df)
    df = check_old(df)
    backup = pd.read_csv('Trending Videos_new.csv')
    os.remove('Trending Videos.csv')
    backup.to_csv('Trending Videos.csv')
